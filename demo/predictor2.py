...
from video_visualizer2 import VideoVisualizer
from visualizer2 import ColorMode, Visualizer
...
class VisualizationDemo(object):
    ...
    def run_on_video(self, video, img, classint, oneorallint):
        """
        ...
        def process_predictions(frame, predictions, img, classint, oneorallint):
            ...
            if "instances" in predictions:
                ...
                if channel == 0:          #predictions의 인스턴스 개수가 없으면 패스
                    ...
                    if oneorallint == 1:
                        ...
                        for i in range(channel): #큰 객체 1개 옵션 선택시 입력받은 마스크들 비교하여 마스크가 큰 객체만 반환
                            tempa1 = moutput.flatten()
                            tempb1 = test[i].flatten()
                            tempa2 = tempa1.tolist()
                            tempb2 = tempb1.tolist()
                            if tempa2.count(True) > tempb2.count(True) :
                                moutput = moutput
                            else:
                                moutput = test[i]
                    else:
        ...
        if self.parallel:
            ...
            for cnt, frame in enumerate(frame_gen):
                ...
                if cnt >= buffer_size:
                    ...
                    yield process_predictions(frame, predictions,img,classint,oneorallint)
            ...
            for frame in frame_gen:
                yield process_predictions(frame, self.predictor(frame),img,classint,oneorallint)
...
class AsyncPredictor:
    ...
    def __init__(self, cfg, num_gpus: int = 1):
        ...
        self.task_queue = mp.Queue(maxsize=num_workers * 4)
        self.result_queue = mp.Queue(maxsize=num_workers * 4)
        self.procs = []
